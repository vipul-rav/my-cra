    ...
    return results.reduce(
        (seq, current) => {
            current.messages.forEach(
                ({ column, line, message, ruleId, severity }) => {
                    const logMessage = {
...
const getFileContext = (filePath, line, column) => {
    ...
    const relativeFilePath = filePathTokens
        .slice(filePathTokens.indexOf('src') + 1)
        .join('/');
...
const createResultDict = summary =>
    [...summary.errors, ...summary.warnings]
        ...
        .reduce((results, { ruleId, type, context }) => {
            results[ruleId] = (() => {
                if (!results[ruleId]) {
...
module.exports = function(results) {
    ...
    const resultList =
        errorCount > 0 || warningCount > 0
            ? dictToList(createResultDict(summary))
            : null;
